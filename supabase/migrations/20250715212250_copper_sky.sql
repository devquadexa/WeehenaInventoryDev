/*
  # Force fix ambiguous product_id column reference
  ...
  2. Solution
    - Force drop the existing function with CASCADE to remove dependencies
    - Recreate with proper column qualification and distinct variable names
    - Ensure all product_id references are qualified as products.product_id
    - Use product_id_result as return variable to avoid conflicts
  ...
  3. Changes
    ...
    - Maintain exact same function logic
*/
...
BEGIN
  ...
  -- Explicitly qualify the product_id column with the table name to avoid ambiguity
  SELECT COALESCE(MAX(
    CAST(
      SUBSTRING(
